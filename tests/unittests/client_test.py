...
class TestInfluxDBClient(TestCase):
    ...
    def test_scheme(self):
        ...
        cli = InfluxDBClient(
            "host", 8086, "username", "password", "database", ssl_usage=True,
            path="somepath"
        )
    ...
    def test_cert(self):
        ...
        cli = InfluxDBClient(ssl_usage=True, cert="/etc/pki/tls/private/dummy.crt")
    ...
    def test_write_points_udp(self):
        ...
        cli = InfluxDBClient(
            "localhost", 8086, "root", "root", "test", use_udp=True, udp_port=port
        )
    ...
    def test_write_points_bad_consistency(self):
        ...
        cli = InfluxDBClient()
        with self.assertRaises(ValueError):
            cli.write_points(self.dummy_points, consistency="boo")
    ...
    def test_get_list_measurements(self):
        ...
        with _mocked_session(self.cli, "get", 200, json.dumps(data)):
            self.assertListEqual(
                self.cli.get_list_measurements(), [{"name": "cpu"}, {"name": "disk"}]
            )
    ...
    def test_create_retention_policy_shard_duration(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            self.cli.create_retention_policy(
                "somename2", "1d", 4, database="db", shard_duration="1h"
            )
    ...
    def test_alter_retention_policy(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            self.cli.alter_retention_policy("somename", "db", shard_duration="1h")
            self.assertEqual(
                m.last_request.qs["q"][0],
    ...
    def test_get_list_users(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            self.assertListEqual(
                self.cli.get_list_users(), [{"user": "test", "admin": False}]
            )
    ...
    def test_create_continuous_query(self):
        ...
        with requests_mock.Mocker() as m:
            m.register_uri(
                requests_mock.GET, "http://localhost:8086/query", text=json.dumps(data)
            )
            query = (
                'SELECT count("value") INTO "6_months"."events" FROM '
    ...
    def test_chunked_response(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            response = self.cli.query("show series", chunked=True, chunk_size=4)
            res = list(response)
    ...
    def test_auth_default(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            self.assertEqual(
                m.last_request.headers["Authorization"], "Basic cm9vdDpyb290"
            )
    ...
    def test_auth_username_password(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            cli = InfluxDBClient(username="my-username", password="my-password")
            cli.ping()
    ...
    def test_auth_token(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            cli = InfluxDBClient(
                username=None, password=None, headers={"Authorization": "my-token"}
            )
            cli.ping()
    ...
    def test_custom_socket_options(self):
        ...
        self.assertEquals(
            cli._session.adapters.get("http://").socket_options, test_socket_options
        )
        self.assertEquals(
            cli._session.adapters.get("http://").poolmanager.connection_pool_kw.get(
                "socket_options"
            ),
            test_socket_options,
        )

        connection_pool = cli._session.adapters.get(
            "http://"
