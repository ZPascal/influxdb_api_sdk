...
if not using_pypy:
    dummy_point_df = {
        ...
        "dataframe": pd.DataFrame(
            [[0.64]], columns=["value"], index=pd.to_datetime(["2009-11-10T23:00:00Z"])
        ),
    }
    dummy_points_df = [
        {
...
class SimpleTests(SingleTestCaseWithServerMixin, unittest.TestCase):
    ...
    def test_create_database(self):
        ...
        self.assertEqual(
            self.cli.get_list_database(), [{"name": "new_db_1"}, {"name": "new_db_2"}]
        )
...
class CommonTests(ManyTestCasesWithServerMixin, unittest.TestCase):
    ...
    def test_write_points_batch(self):
        ...
        net_out = self.cli.query(
            "SELECT value FROM network " "WHERE direction='out'"
        ).raw
    ...
    def test_alter_retention_policy(self):
        ...
        # Test alter duration
        self.cli.alter_retention_policy(
            "somename", "db", duration="4d", shard_duration="2h"
        )
    ...
    def test_create_continuous_query(self):
        ...
        query = (
            'select count("value") into "some_rp"."events" from '
