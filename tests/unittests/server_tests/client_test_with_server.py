...
from tests.unittests import skip_if_pypy, using_pypy, skip_server_tests
from tests.unittests.server_tests.base import ManyTestCasesWithServerMixin
from tests.unittests.server_tests.base import SingleTestCaseWithServerMixin
from tests.unittests.server_tests.base import ManyTestCasesWithServerGzipMixin
from tests.unittests.server_tests.base import SingleTestCaseWithServerGzipMixin
...
if not using_pypy:
    dummy_point_df = {
        "measurement": "cpu_load_short",
        "tags": {"host": "server01", "region": "us-west"},
        "dataframe": pd.DataFrame([[0.64]], columns=["value"], index=pd.to_datetime(["2009-11-10T23:00:00Z"])),
    }
    dummy_points_df = [
        {
            "measurement": "cpu_load_short",
...
class SimpleTests(SingleTestCaseWithServerMixin, unittest.TestCase):
    ...
    def test_create_database(self):
        ...
        self.assertEqual(self.cli.get_list_database(), [{"name": "new_db_1"}, {"name": "new_db_2"}])
    ...
    def test_query_fail(self):
        ...
        self.assertIn("database not found: db", ctx.exception.content)

    def test_query_fail_ignore_errors(self):
        """Test query failed but ignore errors."""
        result = self.cli.query("select column_one from foo", raise_errors=False)
    ...
    def test_create_user(self):
        ...
        self.assertIn({"user": "test_user", "admin": False}, rsp)

    def test_create_user_admin(self):
        """Test create admin user."""
        self.cli.create_user("test_user", "secret_password", True)
        rsp = list(self.cli.query("SHOW USERS")["results"])
        self.assertIn({"user": "test_user", "admin": True}, rsp)
    ...
    def test_drop_user_nonexisting(self):
        ...
        with self.assertRaises(InfluxDBClientError) as ctx:
            self.cli.drop_user("test")
        self.assertIn("user not found", ctx.exception.content)
    ...
    def test_revoke_admin_privileges(self):
        ...
        self.assertEqual([{"user": "test", "admin": True}], self.cli.get_list_users())
        self.cli.revoke_admin_privileges("test")
        self.assertEqual([{"user": "test", "admin": False}], self.cli.get_list_users())
    ...
    def test_grant_privilege_invalid(self):
        ...
        self.assertIn('{"error":"error parsing query: ', ctx.exception.content)
...
class CommonTests(ManyTestCasesWithServerMixin, unittest.TestCase):
    ...
    def test_write_points_check_read_DF(self):
        ...
        assert_frame_equal(rsp["cpu_load_short"], dummy_point_df["dataframe"])

        # Query with Tags
        rsp = self.cliDF.query("SELECT * FROM cpu_load_short GROUP BY *")
        assert_frame_equal(
            rsp[("cpu_load_short", (("host", "server01"), ("region", "us-west")))],
            dummy_point_df["dataframe"],
        )
    ...
    def test_write_multiple_points_different_series_DF(self):
        ...
        assert_frame_equal(rsp["cpu_load_short"], dummy_points_df[0]["dataframe"])

        rsp = self.cliDF.query("SELECT * FROM memory")
        assert_frame_equal(rsp["memory"], dummy_points_df[1]["dataframe"])
    ...
    def test_write_points_batch(self):
        ...
        time.sleep(5)
        net_in = self.cli.query(
            "SELECT value FROM network WHERE direction=$dir",
            bind_params={"dir": "in"},
        ).raw
        net_out = self.cli.query("SELECT value FROM network WHERE direction='out'").raw
    ...
    def test_create_retention_policy(self):
        ...
        # recreate the RP
        self.cli.create_retention_policy("somename", "1w", 1, shard_duration="1h")
    ...
    def test_alter_retention_policy(self):
        ...
        # Test alter duration
        self.cli.alter_retention_policy("somename", "db", duration="4d", shard_duration="2h")
        ...
        self.cli.alter_retention_policy("somename", "db", shard_duration="4h")
    ...
    def test_create_continuous_query(self):
        ...
        query = 'select count("value") into "some_rp"."events" from "events" group by time(10m)'
...
GROUP BY tag_1"
    ...
    def test_get_list_series(self):
        ...
        self.assertEquals(
            self.cli.get_list_series(measurement="cpu_load_short", tags={"host": "server02"}),
            [],
        )
