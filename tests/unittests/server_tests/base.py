...
def _setup_influxdb_server(inst):
    ...
    inst.cli = InfluxDBClient("localhost", inst.influxd_inst.http_port, "root", "", database="db")
    if not using_pypy:
        inst.cliDF = DataFrameClient("localhost", inst.influxd_inst.http_port, "root", "", database="db")


def _setup_gzip_client(inst):
    inst.cli = InfluxDBClient("localhost", inst.influxd_inst.http_port, "root", "", database="db", gzip=True)
