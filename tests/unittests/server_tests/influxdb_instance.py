...
from tests.unittests.misc import is_port_open, get_free_ports
...
class InfluxDbInstance(object):
    ...
    def __init__(self, conf_template, udp_enabled=False):
        ...
        if os.environ.get("INFLUXDB_PYTHON_SKIP_SERVER_TESTS", None) == "True":
            raise SkipTest("Skipping server test (INFLUXDB_PYTHON_SKIP_SERVER_TESTS)")
    ...
    def _start_server(self, conf_template, udp_enabled):
        ...
        tempdir = self.temp_dir_influxdb = tempfile.mkdtemp(dir=self.temp_dir_base)
        ...
        try:
            while time.time() < timeout:
                if is_port_open(self.http_port) and is_port_open(self.global_port):
