...
class TestDataFrameClient(TestCase):
    ...
    def test_write_points_from_dataframe_with_line_of_none(self):
        ...
        expected = b'foo column_one="2",column_two=2.0,column_three=2.0 3600000000000\n'
    ...
    def test_write_points_from_dataframe_with_tag_columns(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            cli.write_points(dataframe, "foo", tag_columns=["tag_one", "tag_two"], tags=None)
            self.assertEqual(m.last_request.body, expected)
    ...
    def test_write_points_from_dataframe_with_numeric_precision(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            cli = DataFrameClient(database="db")
            cli.write_points(dataframe, "foo", {"hello": "there"}, numeric_precision="full")
    ...
    def test_get_list_measurements(self):
        ...
        with _mocked_session(cli, "get", 200, json.dumps(data)):
            self.assertListEqual(cli.get_list_measurements(), [{"name": "cpu"}, {"name": "disk"}])
    ...
    def test_create_retention_policy_default(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            cli.create_retention_policy("somename", "1d", 4, default=True, database="db")
    ...
    def test_multiquery_into_dataframe_dropna(self):
        ...
        with _mocked_session(cli, "GET", 200, data):
            result = cli.query(iql, bind_params=bind_params)
            for r, e in zip(result, expected_dropna_true, strict=True):
                for k in e:
                    assert_frame_equal(e[k], r[k])
    ...
    def test_get_list_database(self):
        ...
        cli = DataFrameClient("host", 8086, "username", "password", "db")
        with _mocked_session(cli, "get", 200, json.dumps(data)):
            self.assertListEqual(cli.get_list_database(), [{"name": "new_db_1"}, {"name": "new_db_2"}])
    ...
    def test_datetime_to_epoch(self):
        ...
        self.assertEqual(cli._datetime_to_epoch(timestamp, time_precision="h"), 1356998400.0 / 3600)
        self.assertEqual(cli._datetime_to_epoch(timestamp, time_precision="m"), 1356998400.0 / 60)
        ...
        self.assertEqual(cli._datetime_to_epoch(timestamp, time_precision="n"), 1356998400000000000.0)
    ...
    def test_write_points_from_dataframe_with_tags_and_nan_line(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            cli.write_points(dataframe, "foo", protocol="line", tag_columns=["tag_one", "tag_two"])
            self.assertEqual(m.last_request.body, expected)
    ...
    def test_write_points_from_dataframe_with_tags_and_nan_json(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            cli.write_points(dataframe, "foo", protocol="json", tag_columns=["tag_one", "tag_two"])
            self.assertEqual(m.last_request.body, expected)
