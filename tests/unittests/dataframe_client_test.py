...
@skip_if_pypy
class TestDataFrameClient(TestCase):
    ...
    def test_write_points_from_dataframe(self):
        ...
        with requests_mock.Mocker() as m:
            m.register_uri(
                requests_mock.POST, "http://localhost:8086/write", status_code=204
            )
    ...
    def test_write_points_from_dataframe_with_tag_columns(self):
        ...
        now = pd.Timestamp("1970-01-01 00:00+00:00")
        dataframe = pd.DataFrame(
            data=[["blue", 1, "1", 1, 1.0], ["red", 0, "2", 2, 2.0]],
            index=[now, now + timedelta(hours=1)],
            columns=["tag_one", "tag_two", "column_one", "column_two", "column_three"],
        )
        expected = (
        ...
        with requests_mock.Mocker() as m:
            ...
            cli.write_points(dataframe, "foo", tag_columns=["tag_one", "tag_two"])
            self.assertEqual(m.last_request.body, expected)

            cli.write_points(
                dataframe, "foo", tag_columns=["tag_one", "tag_two"], tags=None
            )
            self.assertEqual(m.last_request.body, expected)
    ...
    def test_write_points_from_dataframe_with_tag_cols_and_defaults(self):
        ...
        now = pd.Timestamp("1970-01-01 00:00+00:00")
        dataframe = pd.DataFrame(
            data=[["blue", 1, "1", 1, 1.0, "hot"], ["red", 0, "2", 2, 2.0, "cold"]],
            index=[now, now + timedelta(hours=1)],
            columns=[
                "tag_one",
                "tag_two",
        ...
        with requests_mock.Mocker() as m:
            ...
            cli.write_points(
                dataframe,
                "foo",
                field_columns=["column_one", "column_two", "column_three"],
            )
    ...
    def test_write_points_from_dataframe_with_numeric_column_names(self):
        ...
        dataframe = pd.DataFrame(
            data=[["1", 1, 1.0], ["2", 2, 2.0]], index=[now, now + timedelta(hours=1)]
        )
    ...
    def test_write_points_from_dataframe_with_numeric_precision(self):
        ...
        # df with numeric column names
        dataframe = pd.DataFrame(
            data=[["1", 1, 1.1111111111111], ["2", 2, 2.2222222222222]],
            index=[now, now + timedelta(hours=1)],
        )
        ...
        with requests_mock.Mocker() as m:
            ...
            cli = DataFrameClient(database="db")
            cli.write_points(dataframe, "foo", {"hello": "there"}, numeric_precision=4)

            self.assertEqual(m.last_request.body, expected_specified_precision)

            cli = DataFrameClient(database="db")
            cli.write_points(
                dataframe, "foo", {"hello": "there"}, numeric_precision="full"
            )
    ...
    def test_get_list_measurements(self):
        ...
        with _mocked_session(cli, "get", 200, json.dumps(data)):
            self.assertListEqual(
                cli.get_list_measurements(), [{"name": "cpu"}, {"name": "disk"}]
            )
    ...
    def test_get_list_retention_policies(self):
        ...
        with requests_mock.Mocker() as m:
            m.register_uri(
                requests_mock.GET, "http://localhost:8086/query", text=example_response
            )
            self.assertListEqual(
    ...
    def test_query_into_dataframe(self):
        ...
        pd1 = pd.DataFrame(
            [[23422]], columns=["value"], index=pd.to_datetime(["2009-11-10T23:00:00Z"])
        )
        ...
        pd2 = pd.DataFrame(
            ...
            index=pd.to_datetime(
                ["2009-11-10T23:00:00Z", "2009-11-10T23:00:00Z", "2009-11-10T23:00:00Z"]
            ),
        )
    ...
    def test_multiquery_into_dataframe(self):
        ...
        cli = DataFrameClient("host", 8086, "username", "password", "db")
        iql = (
            "SELECT value FROM cpu_load_short WHERE region=$region;"
    ...
    def test_multiquery_into_dataframe_dropna(self):
        ...
        expected_dropna_false = [{"cpu_load_short": pd1}, {"cpu_load_short": pd2}]
        ...
        for dropna in [True, False]:
            with _mocked_session(cli, "GET", 200, data):
                ...
                expected = expected_dropna_true if dropna else expected_dropna_false
                for r, e in zip(result, expected):
                    for k in e:
                        assert_frame_equal(e[k], r[k])
    ...
    def test_get_list_database(self):
        ...
        with _mocked_session(cli, "get", 200, json.dumps(data)):
            self.assertListEqual(
                cli.get_list_database(), [{"name": "new_db_1"}, {"name": "new_db_2"}]
            )
    ...
    def test_datetime_to_epoch(self):
        ...
        self.assertEqual(
            cli._datetime_to_epoch(timestamp, time_precision="h"), 1356998400.0 / 3600
        )
        self.assertEqual(
            cli._datetime_to_epoch(timestamp, time_precision="m"), 1356998400.0 / 60
        )
        self.assertEqual(
            cli._datetime_to_epoch(timestamp, time_precision="s"), 1356998400.0
        )
        self.assertEqual(
            cli._datetime_to_epoch(timestamp, time_precision="ms"), 1356998400000.0
        )
        self.assertEqual(
    ...
    def test_write_points_from_dataframe_with_tags_and_nan_line(self):
        ...
        now = pd.Timestamp("1970-01-01 00:00+00:00")
        dataframe = pd.DataFrame(
            data=[["blue", 1, "1", 1, np.inf], ["red", 0, "2", 2, np.nan]],
            index=[now, now + timedelta(hours=1)],
            columns=["tag_one", "tag_two", "column_one", "column_two", "column_three"],
        )
        expected = (
        ...
        with requests_mock.Mocker() as m:
            ...
            cli.write_points(
                dataframe, "foo", protocol="line", tag_columns=["tag_one", "tag_two"]
            )
            self.assertEqual(m.last_request.body, expected)
    ...
    def test_write_points_from_dataframe_with_tags_and_nan_json(self):
        ...
        with requests_mock.Mocker() as m:
            ...
            cli.write_points(
                dataframe, "foo", protocol="json", tag_columns=["tag_one", "tag_two"]
            )
            self.assertEqual(m.last_request.body, expected)
    ...
    def test_query_custom_index(self):
        ...
        with _mocked_session(cli, "GET", 200, data):
            result = cli.query(
                iql, bind_params=bind_params, data_frame_index=["time", "host"]
            )
