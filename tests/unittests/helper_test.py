...
class TestSeriesHelper(TestCase):
    ...
    def setUpClass(cls):
        ...
        TestSeriesHelper.client = InfluxDBClient(
            "host", 8086, "username", "password", "database"
        )
    ...
    def testSingleSeriesName(self, current_timestamp):
        ...
        self.assertTrue(
            all([el in expectation for el in rcvd])
            and all([el in rcvd for el in expectation]),
    ...
    def testSeveralSeriesNames(self, current_timestamp):
        ...
        self.assertTrue(
            ...
            and all([el in rcvd for el in expectation]),
            "Invalid JSON body of time series returned from "
            "_json_body_ for several series names: {0}.".format(rcvd),
        )
    ...
    def testSeriesWithoutTimeField(self, current_timestamp):
        ...
        TestSeriesHelper.MySeriesHelper(
            server_name="us.east-1", other_tag="ello", some_stat=159, time=current_date
        )
        TestSeriesHelper.MySeriesHelper(
