...
class DataFrameClient(InfluxDBClient):
    ...
    def write_points(
        ...
        super(DataFrameClient, self).write_points(
            points, time_precision, database, retention_policy, protocol=protocol
        )
        ...
        if query.strip().upper().startswith("SELECT"):
            if len(results) > 0:
                return self._to_dataframe(
                    results, dropna, data_frame_index=data_frame_index
                )
            else:
                return {}
        else:
    ...
    def _convert_dataframe_to_json(
        ...
        if not tag_columns:
            points = [
                ...
                for ts, (_, rec) in zip(
                    dataframe.index, dataframe[field_columns].iterrows()
                )
            ]
    ...
    def _convert_dataframe_to_lines(
        self,
        dataframe,
        measurement,
        field_columns=None,
