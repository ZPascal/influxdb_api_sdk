...
class DataFrameClient(InfluxDBClient):
    ...
    def query(self, query, time_precision="s", chunked=False):
        ...
        """
        result = InfluxDBClient.query(
            self, query=query, time_precision=time_precision, chunked=chunked
        )
        if len(result) == 0:
            return result
        elif len(result) == 1:
            ...
            for time_series in result:
                ret[time_series["name"]] = self._to_dataframe(
                    time_series, time_precision
                )
            return ret
    ...
    def _to_dataframe(json_result, time_precision):
        dataframe = pd.DataFrame(
            data=json_result["points"], columns=json_result["columns"]
        )
        ...
        dataframe.index = pd.to_datetime(
            list(dataframe["time"]), unit=pandas_time_unit, utc=True
        )
        del dataframe["time"]
        return dataframe
    ...
    def _convert_dataframe_to_json(self, dataframe, name, time_precision="s"):
        ...
        dataframe["time"] = [
            self._datetime_to_epoch(dt, time_precision) for dt in dataframe.index
        ]
        data = {
            "name": name,
    ...
    def _convert_array(self, array):
        try:
            ...
            raise ImportError(
                "DataFrameClient requires Numpy, "
                '"{ex}" problem importing'.format(ex=str(ex))
            )

        if self.ignore_nan:
            number_types = (int, float, np.number)
            condition = all(isinstance(el, number_types) for el in array) and np.isnan(
                array
            )
