...
class DataFrameClient(InfluxDBClient):
    ...
    def __init__(self, ignore_nan=True, *args, **kwargs):
        ...
        try:
            ...
            raise ImportError('DataFrameClient requires Pandas, "{ex}" problem importing'.format(ex=str(ex))) from ex
    ...
    def write_points(self, data, *args, **kwargs):
        ...
        if batch_size:
            ...
            for key, data_frame in data.items():
                number_batches = int(math.ceil(len(data_frame) / float(batch_size)))
                for batch in range(number_batches):
                    ...
                    outdata = [
                        self._convert_dataframe_to_json(
                            name=key,
                            dataframe=data_frame.iloc[start_index:end_index].copy(),
        ...
        outdata = [
            self._convert_dataframe_to_json(name=key, dataframe=dataframe, time_precision=time_precision)
    ...
    def query(self, query, time_precision="s", chunked=False):
        ...
        """
        result = InfluxDBClient.query(self, query=query, time_precision=time_precision, chunked=chunked)
        if len(result) == 0:
            return result
        elif len(result) == 1:
            ...
            for time_series in result:
                ret[time_series["name"]] = self._to_dataframe(time_series, time_precision)
            return ret

    @staticmethod
    def _to_dataframe(json_result, time_precision):
        dataframe = pd.DataFrame(data=json_result["points"], columns=json_result["columns"])
    ...
    def _convert_dataframe_to_json(self, dataframe, name, time_precision="s"):
        ...
        dataframe["time"] = [self._datetime_to_epoch(dt, time_precision) for dt in dataframe.index]
        data = {
            "name": name,
    ...
    def _convert_array(self, array):
        try:
            ...
            raise ImportError('DataFrameClient requires Numpy, "{ex}" problem importing'.format(ex=str(ex))) from ex

        if self.ignore_nan:
            number_types = (int, float, np.number)
            condition = all(isinstance(el, number_types) for el in array) and np.isnan(array)
